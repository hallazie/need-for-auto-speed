...
def speed_to_vec(speed):
    """
    因为百公里上数值不会超过400KMpH，所以只需要4+10+10=24位
    :param speed:
    :return:
    """
    if not 0 <= speed <= 999:
        return [0 if i != 0 else 1 for i in range(24)]
    speed = str(speed).zfill(3)
    v1 = [1 if str(i) == speed[0] else 0 for i in range(4)]
...
def vec_to_speed(vec):
    s1 = np.argmax(np.array(vec[:4]))
    s2 = np.argmax(np.array(vec[4:14]))
    s3 = np.argmax(np.array(vec[14:24]))
...
if __name__ == '__main__':
    ...
    print(vec_to_speed(speed_to_vec(24)))
